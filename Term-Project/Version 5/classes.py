...
class Player(object):
    def __init__(self, x, y, width, height, beginning, GenresForTheGame):
    ...
    def draw(self, canvas):
        ...
        self.border = (self.x + self.vel, self.y, self.width-70, self.height-15)
        pygame.draw.rect(canvas, self.green, self.border, 2)
    ...
    def accident(self):
        ...
        if self.health > 0:
            ...
            self.alreadyLost = True
            self.beginning.begun = False
            # need to create this lossing image
            # bg = 5
...
class Coin(object):
    ...
    def draw(self, canvas):
        ...
        canvas.blit(coin, [self.x, self.y])
